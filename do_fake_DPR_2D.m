function Fake_DPR_2D_struct = do_fake_DPR_2D(Region, speciesname)

% run DPR calculation for 2D model for North Coast 
% only for testing deletion
...
switch Region
    case 'NCSR'
        load('NCSR/conn_mat_NC_2D.mat')

    case 'SCSR'
        load('SCSR/conn_mat_SC_2D.mat')
end
%keyboard
savename = optimal_results_savename;

%keyboard
Package_Names = fieldnames(res_struct_2D.(Species_Names{1}).pkg); % read out of DPR_setup_opt_file
FLEP = [0 0.1 1]; % have to add the unfished scenario
%keyboard
switch Ocean
...
Fake_DPR_2D_struct = struct([]);
%Species_Names;
%for s = 1:1 % should only be accessing one species at a time, so only needs to run once
    s = speciesname;
    %keyboard
    
    rst = struct;%([]);
    
    HR_matrix = res_struct_2D.(s).HR_matrix;
    
    conn_mat = conn_struct.(s).alt_conn_mat; %CONN MAT
    I = conn_struct.(s).I;
    J = conn_struct.(s).J;
    lookup_table = conn_struct.(s).lookup_table;
    
    habras = res_struct_2D.(s).Habras;
    habvec = res_struct_2D.(s).Habvec(:); %HAB VEC
    
    % Habvec must be in km^2
    
    switch s
        case 'RedSeaUrchin'
        depvec = res_struct_2D.(s).Depvec(:); 
   ...
   %keyboard
%for p = 1:1 % loop across MPA packages 
        P = 'deletion'; %Package_Names{6}; %should retrieve the deletion metric %#6 should be deletion

        rstp = struct([]);
        
        Resvec = res_struct_2D.(s).pkg.(P).resvec(:);
...
for sl = 1:1%length(Species.(s).slope) % loop over different slopes for the species
    ...
    rstp(1).nominal_slope = 1/Species.(s).nominal_thresh(sl); %ends up being 4
...
for f = 1:length(FLEP) % loop across FLEPs
    ...
    % Add a kluge to constrain urchin fishing to shallow water
        switch s
            case 'RedSeaUrchin'
                ...
                F_total = sum((habvec>0).*(depvec>0).*Species.(s).fishing(f));
            ...
            case 'DungenessCrab' % only have 1 fishing level
                ...
                F_total = sum((habvec>0).*(depvec>0).*Species.(s).fishing(1));
                Area_total = sum((habvec>0).*(depvec>0).*(1-Resvec));
                F_spatial = (depvec>0).*(habvec>0).*(1-Resvec).*F_total./Area_total;
                
            otherwise
                F_total = sum((habvec>0).*Species.(s).fishing(f));
    ...
    F_summarized = HR_matrix*F_spatial(:);
     ...
     switch Ocean
        ...
        [eggs,settlers,recruits,selfers] = calculate_dpr( Species.(s).slope(sl), habvec, FLEP_vec(I), conn_mat(I,I),100, rect_type );
...
%keyboard            
Species.(s).Species_Name = speciesname;
            [N_init, R_init S_init SR_init Y_init, B_init] = popmodel(N_init,Y_init,conn_mat(I,I,:),noF_spatial,habvec(:),Species.(s),slope_2,100,rect_type);
            N_init = N_init(:,:,end);
            B_init = B_init(:,:,end);
            Y_init = Y_init(:,:,end);    
            ...
            [N R S SR Y B E] = popmodel(N_init, Y_init, conn_mat(I,I,:), F_vec(:,i-1), habvec(:), Species.(s),slope_2,2,rect_type);  
                % N is n x age x time
                % R, S are n x time
                % Y, B are n x sizeclass x time
...
Fake_DPR_2D_struct(1).(s).pkg = rst;

clear habras habvec HR_matrix  rst

%end % end loop over species
  %{    
 Savename = 'Fake_DPR_2D_struct';                               
save(Savename, 'Fake_DPR_2D_struct','-v7.3');% save DPR_2D_struct
clear all
%}